...
const STATUS_CONFIG = {
  ...
  untested: {
    label: "Untested",
    icon: Play,
    color: "text-slate-300 bg-white border-slate-200",
    badge: "badge",
  },
};
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="card w-full max-w-md p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="label">Suite (optional)</label>
            <select
              className="input"
              value={form.suite}
              onChange={(e) =>
                setForm((f) => ({ ...f, suite: e.target.value }))
              }
            >
...
export default function TestRunPage() {
  ...
  const updateResult = async (testCaseId, status, notes = "") => {
    const { data } = await api.put(
      `/runs/${activeRun._id}/results/${testCaseId}`,
      { status, notes },
    );
    setActiveRun(data);
  ...
  // const handleDeleteRuns = async (id) =>{
  //   try{
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          {isSelected &&(
            <div className="flex">
              <button className="secondary-btn bg-red-500 text-white hover:bg-red-500 btn-ghost py-1.5 text-sm">
                Delete Run
              </button>
            </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 card p-10">
          ...
          <p className="text-slate-400 text-sm mb-4">
            Create your first test run to start executing
          </p>
          <button
              ...
              <div key={run._id} className="card overflow-hidden">
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-2 h-2 rounded-full ${run.status === "completed" ? "bg-emerald-500" : run.status === "in_progress" ? "bg-blue-500" : "bg-slate-300"}`}
                    />
                    <div>
      ...
      {showModal && (
        <NewRunModal
          ...
          onCreated={(r) => setRuns((prev) => [r, ...prev])}
        />
      )}
    </div>
  );
