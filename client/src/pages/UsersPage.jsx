...
import { Users, Shield, Edit, Trash2, CheckCircle2, XCircle } from 'lucide-react';
import toast from 'react-hot-toast';

const ROLE_COLORS = { admin: 'bg-purple-100 text-purple-700', manager: 'bg-blue-100 text-blue-700', tester: 'bg-green-100 text-green-700' };
          ...
          <tbody className="divide-y divide-slate-100">
            {loading ? (
              ...
              <tr key={u._id} className="hover:bg-slate-50 group">
                ...
                <td className="px-5 py-4">
                  {editing === u._id ? (
                    <select defaultValue={u.role} className="input text-xs py-1 w-28"
                      onChange={e => handleUpdate(u._id, { role: e.target.value })}>
                      {['admin','manager','tester'].map(r => <option key={r} value={r} className="capitalize">{r}</option>)}
                    </select>
                  ) : <span className={`badge capitalize ${ROLE_COLORS[u.role]}`}>{u.role}</span>}
                </td>
                <td className="px-5 py-4">
                  <button onClick={() => handleUpdate(u._id, { isActive: !u.isActive })}
                ...
                <td className="px-5 py-4">
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => setEditing(editing === u._id ? null : u._id)}
