...
const projectsList = async (req, res) =>{
    try{
        ...
        const projects = await Project.find(query).populate("createdBy", "name email avater").populate('members.user', 'name email avater').sort({ updatedAt: -1 });
...
const getProjectById = async (req, res) =>{
    try{
        ...
        const project = await Project.findById(projectId).populate('createdBy', 'name email avatar').populate('members.user', 'name email avatar');
...
module.exports = {canManage, projectsList, getProjectById, createProject, updateProjectById, deleteProjectById, addMembersToProject};